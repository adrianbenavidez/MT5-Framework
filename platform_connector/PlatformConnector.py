...
class PlatformConnector():
    ...
    def _initialize_platform(self) -> None:
        ...
        if mt5.initialize(
            ...
            raise Exception(
                f"Ha ocurrido un error al lanzar la plataforma MT5: {mt5.last_error()}")
    ...
    def _check_algo_trading_enabled(self) -> None:
        ...
        if not mt5.terminal_info().trade_allowed:
            raise Exception(
                "El trading algorítmico está desactivado. Por favor, actívalo MANUALMENTE. Tools -> Options -> Expert Advisors -> Allow algorithmic trading")
    ...
    def _add_symbols_to_marketwatch(self, symbols: list) -> None:
        ...
        for symbol in symbols:
            ...
            if mt5.symbol_info(symbol) is None:
                print(
                    f"No se ha podido añadir el símbolo {symbol} al MarketWatch: {mt5.last_error()}")
                continue
            ...
            if not mt5.symbol_info(symbol).visible:
                if not mt5.symbol_select(symbol, True):
                    ...
                    print(
                        f"El símbolo {symbol} se ha añadido con éxito al MarketWatch!")
            else:
